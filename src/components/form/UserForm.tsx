    ...
    <>
      ...
      >
        ...
        <textarea
          id="message"
          name="message"
          className="resize-none h-56 text-black flex rounded-md shadow-sm ring-1 ring-inset ring-gray-300 focus-within:ring-2 focus-within:ring-inset focus-within:ring-indigo-600"
        />
        <button
          type="submit"
          disabled={pending}
          className={`w-full bg-slate-300 text-black h-10 rounded-md ${
            pending ? "bg-slate-100" : "bg-slate-300"
          }`}
        >
          submit
        </button>
      </form>
      {state?.error && <p className="text-white">{state?.error.email} 😭</p>}
